...
function MovieTrailer() {
  ...
  // console.log(data);
  return (
    <div className="movie-trailer mb-10">
      <h4 className="text-lg mb-5 text-white uppercase font-bold">Trailer</h4>
...
function MovieSimilar() {
  ...
  return (
    <div className="movie-similar">
      <h4 className="text-lg mb-5 text-white uppercase font-bold">
        Similar movies
      </h4>
      <Swiper grabCursor="true" slidesPerView="auto" spaceBetween={10}>
        {results.length > 0 &&
          results.map((item) => (
            <SwiperSlide grabCursor="true">
              <div className="flex flex-col items-start" onClick={()}>
                <img
                  src={`https://image.tmdb.org/t/p/original/${item.poster_path}`}
                  alt=""
                />
                <p className="text-sm text-white p-1">{item.title}</p>
              </div>
            </SwiperSlide>
          ))}
      </Swiper>
